...
$fontface = 'FZYTK.TTF'; //图片字体，从系统控制面板字体中选择拷贝出来
//定义文字库
$str = "从系统控制面板字体中选择拷贝出来声名变量在服务器端保存每次生成定义绘制起点坐标的验证码然后在该脚本里就可以使用开始绘制使用完就销毁";
//定义中文验证码库
$strdb = str_split($str, 3); //将$str按每三个字符切割成数组，因为在utf-8中每三个字符代表一个汉子
...
for ($i = 0; $i < 200; $i++) {
  ...
  imagesetpixel($image, rand(1, 199), rand(1, 59), $pointcolor); //imagesetpixel方法将点画到画布上
}

//设置背景干扰元素-线(3条干扰线即可)
for ($i = 0; $i < 3; $i++) {
  $linecolor = imagecolorallocate($image, rand(80, 220), rand(80, 220), rand(80, 220));
  imageline($image, rand(1, 199), rand(1, 59), rand(1, 199), rand(1, 59), $linecolor); //imageline方法将线绘制到画布上
}
